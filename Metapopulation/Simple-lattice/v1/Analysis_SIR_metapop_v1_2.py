        ...
        for (beta, mu) in zip(beta_vals, mu_vals):
            ...
            for sim in range(nbr_simulations):
                if sim not in idx_sims_not_start:
                    ...
                    plt.ylabel(r'$\Delta I$ / $\Delta R$', fontsize = 18)
                    ax.tick_params(axis='both', which='major', labelsize=16)  # Adjust the size of major ticks
            ...
            print('idx I_new:', idx_max_Inew_varyR0)

            print('avg t I max:', np.array(idx_max_I_varyR0).mean())
