...
###### OUTBREAK CASE ######
    ...
    for row, col in zip(N_row, N_col):
        ...
        for beta, mu in zip(beta_outbreak, mu_outbreak):
            ...
            ani = animation.FuncAnimation(fig, animate, fargs=(img, grid, dict_outbreak_values, node_population_time, N, ),
                                          frames= dict_outbreak.keys())
    ...
    for row, col in zip(N_row, N_col):
        ...
        for beta, mu in zip(beta_no_outbreak, mu_no_outbreak):
            ...
            # Setup animation
            fig, ax = plt.subplots()
            img = ax.imshow(grid,  vmin = 0, vmax = 0.3, cmap = 'coolwarm')
            fig.colorbar(img, cmap = 'coolwarm')
            ax.set_xlabel('Node index')
            ax.set_ylabel('Node index')
...
for row, col in zip(N_row, N_col):
    ...
    for beta, mu in zip(beta_outbreak, mu_outbreak):
        ...
        ax.set_title(f"PE for outbreak data, {N_row}x{N_col}, beta = {beta}, mu = {mu}: sim {sim}")
        #ax.legend()
        #plt.show()
